// elapsedMillis_timeout.ino
// status: compile ok, upload ok
// but sketch is not working
// sth wrong with Serial.read()


/*
  timeouts.ino
  Using timeouts for user interaction
  A more "real" example than blinking a LED
  demonstrate waiting for user input while 
  ...
  Note that elapsedMillis variables should usually have global scope.
  If they're local, they restart from zero every time the function runs,
  which might be useful in some cases, but usually you want them to retain 
  their value over longer time.  
*/


#include <elapsedMillis.h>  // still need .h?
#include <ctype.h>
...
void loop() 
{
  ...
  case WAITING:
      ...
      if (validInput.indexOf(userinput) == -1)    // sth wrong here?
      {
        state = ERROR;
        Serial.println(validInput.indexOf(userinput));
  ...
  case TIMEOUT:
    // If you wanted to be obnoxious, you could ring a bell
    // to get the user's attention at this point
  ...
  {
    // Demonstrate doing 2 things at once - waiting for input (above)
    // while flashing a LED in a sequence (here)
