// gsi_v5.ino
// glide slope indicator system, changi
// compile error caused by lcd.begin()
...
//  analogWrite(PWM1_UD,speed_ud);
  analogWrite(LIMIT_UP,speed_ud);
  analogWrite(LIMIT_DW,0);
  }
void run_ud_stop_motor(){
  speed_ud = 10;
//  analogWrite(PWM1_UD,speed_ud);
  analogWrite(LIMIT_UP,0);
  analogWrite(LIMIT_DW,0);
  }
void run_dw_motor(){
...
//  analogWrite(PWM2_LR,speed_lr);
  analogWrite(LIMIT_LE,speed_lr);
  analogWrite(LIMIT_RT,0);
  }    
void run_lr_stop_motor(){  
  speed_lr = 10;
//  analogWrite(PWM2_LR,speed_lr);
  analogWrite(LIMIT_LE,0);
  analogWrite(LIMIT_RT,0);
  }    
void run_rt_motor(){  
  if(millis()-lasttime_speed_lr>1){
