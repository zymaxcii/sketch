...
#define ENCB_UD 3            // 编码器上下方向B mega2560的pin位 只能使用支持Interrupt——pin：2,3,18,19,20,21   20 and 21 已经用于I2C口 for LCD2004 
#define ENCA_LR 41           // 编码器左右方向A mega2560的pin位
#define ENCB_LR 18           // 编码器左右方向B mega2560的pin位 只能使用支持Interrupt——pin：2,3,18,19,20,21   20 and 21 已经用于I2C口 for LCD2004
...
void loop()
{
 read_limitsw();                        // 读取 limitswitch
 read_button();                         // 读取 button
 readENCODER();                         // 读取 Encoder
 run_motor();                           // 驱动 motor                        
 lcd_display();                         // lcd2004 屏显
}
  ...
  while(1)
  {
    ...
    if (limit_zero_lr_state == 1-STOP)
    {
      if (millis()-lasttime_00lr < ROUND_TIME *1000)
      {
        ...
        if (millis()-lasttime_00lr < ROUND_TIME *2000)
        {
          run_rt_motor();
        }
        else
  ...
  while(1)
  {
    ...
    if (limit_zero_ud_state == 1-STOP)
      ...
      if (millis()-lasttime_00ud < ROUND_TIME *1000)
      {
        ...
        if (millis()-lasttime_00ud <  ROUND_TIME *2000)
        {
          run_dw_motor();
        }
        else
