// universal_acmotor_pid_control.ino
// won't be able to compile it
// just for reading experience - mty
//
// https://create.arduino.cc/projecthub/saulius-bandzevicius/arduino-based-universal-ac-motor-speed-controller-a4ceaf?ref=platform&ref_id=424_trending___&offset=1
...
#define PULSE 2            // number of triac trigger pulse width counts. One count is 16 ms
...
// define PID variables and parameters
...
void loop()
{
  ...
  if (loopflag == true)
  {
    ...
    for (int j = 1; j <= i; j++)
    {
      ...
      myPID.Compute();
      dimming = map(Output, minoutputlimit, maxoutputlimit, maxoutputlimit, minoutputlimit);
      // inverse the output
      dimming = constrain(dimming, mindimminglimit, maxdimminglimit);
      // check that dimming is in 20-625 range
      tempcounter++;
